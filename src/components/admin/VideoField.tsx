...
/**
 * Video URL field with Cloudinary upload, media library picker, and preview.
 *
...
export function VideoField({ label, value, onChange, hint }: VideoFieldProps) {
  ...
  function onFileSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > MAX_VIDEO_UPLOAD_BYTES) {
      setError(
        `File exceeds ${MAX_VIDEO_UPLOAD_LABEL} (${(file.size / (1024 * 1024)).toFixed(1)}MB)`,
      );
      return;
    }
  ...
  const fieldId = `video-field-${label.replace(/\s+/g, "-").toLowerCase()}`;
